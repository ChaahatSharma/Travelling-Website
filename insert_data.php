...
$sql = "SELECT name, trans_mode, time, email, phone, gender FROM user_info WHERE date='$date' AND source='$source' AND destination='$dest' /*AND DATEDIFF(MINUTE,$time,time) < 30 ";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "Name: " . $row["name"]. " - Travel Mode: " . $row["trans_mode"]. " - Time: " . $row["time"]. " - E-Mail: " . $row["email"]. " - Contact Number: " . $row["phone"]. " - Gender: " . $row["gender"]. "<br>";
    }
} else {
